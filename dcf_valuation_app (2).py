...
# ═══════════════════════════════════════════════════════════════
# DATA LAYER — Yahoo Finance crumb auth (works on Streamlit Cloud)
# ═══════════════════════════════════════════════════════════════
...
def get_crumb_and_cookies():
    """Get a valid crumb + session cookies from Yahoo Finance."""
    session = requests.Session()
    session.headers.update(HEADERS)
    # Hit the consent/login page to get cookies
    ...
    return crumb, dict(session.cookies)


@st.cache_data(ttl=300, show_spinner=False)
def fetch_yahoo(ticker: str) -> dict:
    """
    Fetch all needed modules from Yahoo Finance v10 quoteSummary
    using the crumb-based auth that Yahoo now requires.
    """
    crumb, cookies = get_crumb_and_cookies()
    modules = (
    ...
    if not result:
        error = resp.json().get("quoteSummary", {}).get("error", {})
        raise ValueError(error.get("description", "No data returned."))
    return result[0]


@st.cache_data(ttl=300, show_spinner=False)
def fetch_history(ticker: str) -> pd.DataFrame:
    """Fetch 2-year daily price history."""
    crumb, cookies = get_crumb_and_cookies()
    url = (
        f"https://query1.finance.yahoo.com/v8/finance/chart/{ticker}"
        f"?range=2y&interval=1d&crumb={crumb}"
    )
    resp = requests.get(url, headers=HEADERS, cookies=cookies, timeout=12)
    resp.raise_for_status()
...
current_price = raw(price_mod, "regularMarketPrice") or 0.0
...
cash_      = raw(bal_stmts[0], "cash")       if bal_stmts else 0
