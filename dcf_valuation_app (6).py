...
def compute_historical_fcf_growth(cf) -> dict:
    """
    Derives YoY FCF growth rates from up to 4 years of cash-flow data.
    Returns a dict with:
      - rates      : list of growth rates (most recent first, oldest last)
      - avg_3yr    : 3-year average (or fewer if not enough data)
      - cagr       : CAGR over the available period
      - years      : list of calendar years covered
      - fcfs       : raw FCF values ($M) most-recent first
      - clipped_hi : suggested near-term default (clamped 0–40%)
      - clipped_md : suggested mid-term default  (clamped 0–30%)
    """
    fcfs, yrs = [], []
    try:
    ...
    for i in range(len(fcfs) - 1):
        denom = fcfs[i + 1]
        if denom and denom != 0 and fcfs[i] > 0 and denom > 0:
            rates.append((fcfs[i] - denom) / abs(denom))

    avg_3yr = float(np.mean(rates[:3])) if rates else 0.10
    n       = len(fcfs)
    cagr    = (fcfs[0] / fcfs[-1]) ** (1 / (n - 1)) - 1 if n >= 2 and fcfs[-1] > 0 else avg_3yr

    # Sensible slider defaults: cap extreme values
    clipped_hi = float(np.clip(avg_3yr,    -0.10, 0.40))
    clipped_md = float(np.clip(avg_3yr * 0.65, -0.10, 0.30))
    ...
    return dict(
        ...
        clipped_md=clipped_md,
    )


def fetch_analyst_growth(info: dict) -> dict:
    """Pull forward-looking growth estimates from yfinance info dict."""
    return dict(
        eps_next_yr   = info.get("earningsGrowth"),         # trailing 12-mo earnings growth
        revenue_growth= info.get("revenueGrowth"),          # trailing revenue growth
        peg           = info.get("trailingPegRatio"),
        fwd_eps_growth= info.get("earningsQuarterlyGrowth"),
...
# ── Growth intelligence ───────────────────────────────────────
hist_growth   = compute_historical_fcf_growth(cf)
analyst_growth = fetch_analyst_growth(info)
...
The sliders are pre-populated with <b>actual historical growth rates</b> for this company —
you can adjust them freely.
</div>""", unsafe_allow_html=True)

# ── Growth Intelligence Panel ──────────────────────────────────
with st.expander(f"📊 {ticker} Actual Growth Data — expand to see before setting assumptions", expanded=True):
    ...
    with gi1:
        ...
        hg = hist_growth
        if hg["rates"]:
            for i, (rate, yr) in enumerate(zip(hg["rates"], hg["years"])):
                label = f"{hg['years'][i+1] if i+1 < len(hg['years']) else '?'} → {yr}"
                color = "#28a745" if rate >= 0 else "#dc3545"
                st.markdown(
                    f"<span style='color:{color};font-weight:bold'>"
                    f"{label}: {rate*100:+.1f}%</span>",
                    unsafe_allow_html=True
                )
            st.markdown("---")
            st.markdown(f"**3-yr Avg FCF Growth:** `{hg['avg_3yr']*100:.1f}%`")
            st.markdown(f"**FCF CAGR ({hg['years'][-1]}–{hg['years'][0]}):** `{hg['cagr']*100:.1f}%`")
        else:
            st.info("Not enough FCF history to calculate growth rates.")

    # Analyst / forward-looking estimates
    with gi2:
        ...
        rows_ag = {
            "Earnings Growth (TTM)":   pct_or_na(ag["eps_next_yr"]),
            "Revenue Growth (TTM)":    pct_or_na(ag["revenue_growth"]),
            "Quarterly EPS Growth":    pct_or_na(ag["fwd_eps_growth"]),
            "Analyst Price Target":    f"${ag['analyst_target']:.2f}" if ag["analyst_target"] else "N/A",
            "Trailing PEG Ratio":      f"{ag['peg']:.2f}x" if ag["peg"] else "N/A",
        }
        for label, val in rows_ag.items():
            st.markdown(f"**{label}:** `{val}`")

    # FCF history chart
    with gi3:
        st.markdown("#### 🗂️ FCF History ($M)")
        if hg["fcfs"] and hg["years"]:
            # Reverse so chart goes oldest → newest
            chart_yrs  = list(reversed(hg["years"]))
            chart_fcfs = list(reversed(hg["fcfs"]))
            colors_bar = ["#28a745" if v >= 0 else "#dc3545" for v in chart_fcfs]
            fig_hg = go.Figure()
            fig_hg.add_bar(
                x=[str(y) for y in chart_yrs],
                y=chart_fcfs,
                marker_color=colors_bar,
                text=[f"${v:,.0f}M" for v in chart_fcfs],
    ...
    # Suggested defaults callout
    hi_default = round(hg["clipped_hi"] * 100, 1)
    md_default = round(hg["clipped_md"] * 100, 1)
    st.markdown(f"""<div class="metric-box">
    💡 <b>Suggested defaults based on {ticker}'s history:</b>
    &nbsp; Near-term (Yrs 1–3): <b>{hi_default}%</b>
    &emsp;|&emsp; Mid-term (Yrs 4+): <b>{md_default}%</b>
    &emsp;|&emsp; <em>Sliders below are pre-set to these values — adjust freely.</em>
    </div>""", unsafe_allow_html=True)
...
with cg1:
    ...
    use_custom = st.toggle("Custom rate per year (override per-year)")
    growth_rates = []
    if use_custom:
        cols = st.columns(n_years)
        for i, c in enumerate(cols):
            default_g = hi_default if i < 3 else md_default
            g = c.number_input(f"Yr {i+1} (%)", -30.0, 50.0,
                                default_g, 0.5, key=f"g{i}")
            growth_rates.append(g / 100)
    else:
        g_hi = st.slider(
            f"Near-term growth Yrs 1–3 (%) — {ticker} hist avg: {hi_default}%",
            -10.0, 40.0, hi_default, 0.5,
            help=f"Historical 3-yr avg FCF growth for {ticker}: {hg['avg_3yr']*100:.1f}%"
        ) / 100
        g_md = st.slider(
            f"Mid-term growth Yrs 4+ (%) — suggested: {md_default}%",
            -10.0, 30.0, md_default, 0.5,
            help="Typically tapers toward terminal growth rate."
        ) / 100
