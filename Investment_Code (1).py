...
if fetch_btn and ticker:
    if not api_key:
        st.error("⚠️ No Anthropic API key found. Add it to Streamlit Secrets as ANTHROPIC_API_KEY, or paste it in the sidebar.")
    else:
        try:
...
""", unsafe_allow_html=True)
    ...
    st.info("👆 Enter a ticker symbol, then click **Fetch Live Data** to begin.")
